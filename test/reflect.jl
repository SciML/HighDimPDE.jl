...
@testset "CPU reflect methods" begin
    ...
    X0 = fill(0.0, d)
    X1 = X0 + randn(d)
    X11 = HighDimPDE._reflect(X0, X1, fill(-1.0, d), fill(1.0, d))
    ...
    @testset "testing NaN reflect" begin
        ...
        for i in 1:1000
            ...
            y0 .= y1
            y1 .= y0 .+ dW
            y1 .= HighDimPDE._reflect(y0, y1, fill(-1.0, d), fill(1.0, d))
        end
...
if CUDA.functional()
    ...
    @testset "GPU reflect methods" begin
        ...
        X11 = HighDimPDE._reflect(X0, X1, fill(-1.0f0, d), fill(1.0f0, d))
        ...
        @testset "testing NaN reflect" begin
            ...
            for i in 1:100
                ...
                y0 .= y1
                y1 .= y0 .+ dW
                y1 .= HighDimPDE._reflect(y0, y1, -1.0f0, 1.0f0)
            end
            @test count(isnan.(y1)) == 0
        end
    end
