...
@testset "MLP - heat equation - single thread" begin # solving at one unique point
    for d in [1, 10, 50]
        ...
        tspan = (0.0e0, 5.0e-1)
        dt = 5.0e-1  # time step
        μ(x, p, t) = 0.0e0 # advection coefficients
        σ(x, p, t) = 1.0e-1 # diffusion coefficients
        ...
        f(y, v_y, ∇v_y, p, t) = 0.0e0 .* v_y #TODO: this fix is not nice
...
@testset "MLP - heat equation - single thread - Neumann BC" begin # solving at one unique point
    ...
    for d in [1, 2, 5]
        u1s = []
        for _ in 1:2
            neumann_bc = [fill(-5.0e-1, d), fill(5.0e-1, d)]
            g(x) = sum(x .^ 2)
            # d = 10
            x0 = fill(3.0e-1, d)
            alg = MLP()
...
@testset "MLP - allen cahn local, Neumann BC" begin
    ...
    for d in [1, 2, 5]
        neumann_bc = [fill(-5.0f-1, d), fill(5.0f-1, d)]
...
@testset "MLP - replicator mutator" begin
    ...
    σ_sampling = 1.0e0
    μ(x, p, t) = 0.0e0 # advection coefficients
    σ(x, p, t) = 1.0e-1 #1f-1 # diffusion coefficients
    ss0 = 1.0e-2 #std g0
    ...
    for d in [1, 2, 5]
        ...
        g(x) = (2 * π)^(-d / 2) * ss0^(-d * 5.0e-1) * exp.(-5.0e-1 * sum(x .^ 2.0e0 / ss0)) # initial condition
        m(x) = -5.0e-1 * sum(x .^ 2)

        function f(y, z, v_y, v_z, ∇v_y, ∇v_z, p, t)
            max(0.0, v_y) * (
                m(y) -
                    max(0.0, v_z) * m(z) * (2.0 * π)^(d / 2) * σ_sampling^d *
                    exp(0.5 * sum(z .^ 2) / σ_sampling^2)
            )
